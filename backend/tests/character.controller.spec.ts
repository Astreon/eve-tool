...
vi.mock('../src/lib/redis.js', () => {
  ...
  function get(key: string) { return alive(key) ? (store.get(key) ?? null) : null }
  ...
  function exists(key: string) { return alive(key) ? 1 : 0 }
  function del(key: string) { const ok = alive(key); store.delete(key); exp.delete(key); return ok ? 1 : 0 }
  function __flushAll() { store.clear(); exp.clear() }
...
vi.mock('../src/lib/logger.js', () => ({
  logger: {
    info: () => {}, error: () => {},
    entityFromRedis: () => {}, entityFromDb: () => {}, entityFromEsi: () => {},
  }
}))
