...
export function makeCachedController<TDb, TApi, TEsi>(
    ...
    const fallbackTtl =
        cfg.fallbackTtlSec ?? config.esiApi.esiFallbackTtlSeconds
    ...
    return async function handler(
        ...
        try {
            ...
            if (
                dbRow &&
                dbMeta.expiresAt &&
                dbMeta.expiresAt.getTime() > Date.now()
            ) {
                const api = cfg.mapToApi(dbRow)
                const ttlFromDb = Math.max(
                    Math.floor(
                        (dbMeta.expiresAt.getTime() - Date.now()) / 1000,
                    ),
                    0,
                )
                ...
                logger.entityFromDb(cfg.kind, id, {
                    lastUpdated: dbMeta.lastModified
                        ? dbMeta.lastModified.toISOString()
                        : undefined,
                    durationMs: Date.now() - started,
                })
            ...
            if (!lockHeld) {
                ...
                logger.info(
                    cfg.kind,
                    `lock busy & no cache -> continue without lock for ID=${id}`,
                )
            }
                ...
                if (esi.data === null) {
                    ...
                    if (cfg.bumpDbMetaOn304) {
                        await cfg.bumpDbMetaOn304(id, {
                            ...
                            expiresAt: esi.expires
                                ? new Date(esi.expires)
                                : (dbMeta.expiresAt ?? null),
                        })
                    }
